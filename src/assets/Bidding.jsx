...
function Bidding({ closeModal, itemId }) {
  ...
  async function fetchPercentage() {
    ...
    const response = await fetch(
      `${import.meta.env.REACT_APP_MY_API_ENDPOINT}investment/exchange_percentage`,
      {
        method: "POST",
        headers: {
          "Content-type": "application/json",
  ...
  async function bid(id) {
    ...
    const token = localStorage.getItem("user-token");
    const response = await fetch(
      `${import.meta.env.REACT_APP_MY_API_ENDPOINT}exchange/bid_investment`,
      {
        method: "POST",
      ...
      >
        ...
        <>
          <motion.div
            ...
            className={`bg-white rounded-xl border w-full lg:w-1/2 top-28 absolute lg:top-12 ${isClick ? "hidden" : "block"
              }`}
          >
