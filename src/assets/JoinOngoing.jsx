...
function JoinOngoing({ closeModal, itemId, productDetails }) {
  ...
  async function fetchPercentage() {
    ...
    const response = await fetch(
      `${import.meta.env.REACT_APP_MY_API_ENDPOINT}investment/investment_percentage`,
      {
        method: "POST",
        headers: {
          "Content-type": "application/json",
...
export function Warning({
  ...
  async function invest() {
    ...
    const token = localStorage.getItem("user-token");
    const response = await fetch(
      `${import.meta.env.REACT_APP_MY_API_ENDPOINT}investment/join_ongoing_investment`,
      {
        method: "POST",
