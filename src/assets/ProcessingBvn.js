    ...
    <>
      ...
      >
        <motion.div
          ...
          className={`w-128 bg-white rounded-xl absolute border-green p-6 text-center ${bvnCheck ? "hidden" : "block"}`}
        >
          <div>
      ...
      >
        ...
        <div className="input mb-3">
          <input required type="tel" placeholder="enter BVN" className="box" value={bvn} onChange={event => setBvn(event.target.value)}/>
        </div>
        <div className="flex justify-center">
          <button className="rounded-full bg-green w-full h-12 text-dashbg flex justify-around items-center" onClick={() => {
            validateBVN()
      ...
      >
        ...
        {bvnResult?.status === "error" ? 
          <p>Something went wrong, confirm if you submitted the correct BVN</p>
        :
          <p>Sorry! Only bank accounts linked with your BVN can be used for withdrawals.</p>
         }
        </div>
        
        <div className="flex justify-around">
          <button className="rounded-full border text-neutral font-medium text-base px-6" onClick={() => {
            setVerifyBVN(false)
          }}>
            Cancel Withdrawal
          </button>
          {bvnResult?.status === "error" ? <button className="rounded-full bg-green h-12 text-dashbg flex justify-around items-center px-12" onClick={() => {
            setError(false)
            setBvnCheck(true)
          }}>
            Retry
          </button> : <button className="rounded-full bg-green h-12 text-dashbg flex justify-around items-center px-6" onClick={() => {
            // validateBVN()
