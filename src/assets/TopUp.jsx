    ...
    <>
      ...
      >
        ...
        <motion.div
          ...
          className={`bg-white rounded-xl w-full lg:w-1/2 absolute top-28 lg:bottom-auto lg:top-12 ${isClick ? "hidden" : "block"
            }`}
        >
                ...
                .map((post) => (
                  <div className="lg:px-10 px-5">
                    ...
                    <div className="lg:text-right flex lg:block text-center pt-5 pb-8">
                      <button
                        className={`rounded-full w-44 h-12 text-sm lg:text-base text-dashbg bg-red mr-5 ${post.user_id == userID ? "opacity-100" : "opacity-30"
                          }`}
                        onClick={() => {
                          setAuthCancel(true);
                          closeModal(false);
  ...
  return (
    <>
      <motion.div
        ...
        className={`lg:w-128 w-11/12 bg-white rounded-xl fixed top-32 border-green p-6 text-center ${warning ? "block" : "hidden"
          } `}
      >
        <div>
